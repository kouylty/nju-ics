...
<img src="https://raw.githubusercontent.com/kouylty/nju-ics/master/notes/debug2(1).png" style="zoom:50%;" alt="【若图片无法加载，详见https://github.com/kouylty/nju-ics/blob/master/notes/debug2(1).png】">
...
<img src="https://raw.githubusercontent.com/kouylty/nju-ics/master/notes/test2(1).png" style="zoom:30%;" alt="【若图片无法加载，详见https://github.com/kouylty/nju-ics/blob/master/notes/test2(1).png】"> <img src="https://raw.githubusercontent.com/kouylty/nju-ics/master/notes/test2(2).png" style="zoom:30%;" alt="【若图片无法加载，详见https://github.com/kouylty/nju-ics/blob/master/notes/test2(2).png】">

<img src="https://raw.githubusercontent.com/kouylty/nju-ics/master/notes/test2(3).png" style="zoom:30%;" alt="【若图片无法加载，详见https://github.com/kouylty/nju-ics/blob/master/notes/test2(3).png】"> <img src="https://raw.githubusercontent.com/kouylty/nju-ics/master/notes/test2(4).png" style="zoom:30%;" alt="【若图片无法加载，详见https://github.com/kouylty/nju-ics/blob/master/notes/test2(4).png】">
...
在当前 Unix 系统中，目标文件通常以 ELF（executable and linkable format）存储。在内核（kernel）中，我们要做的就是把 ELF 文件加载到内存中。

每个 ELF 文件主要包括三部分：ELF头、程序头表、其余文件体。ELF 头主要记录：该 ELF 文件的类型（常见的是 `ET_REL`，可重定位文件，relocatable）和程序头表的属性（偏移量、表项个数、表项大小）等。程序头表主要记录：段（segment）的类型和地址、文件大小和运行时内存大小等。

在运行 ELF 文件时，内核会把只读代码段、读/写段映射到运行时内存段，在这之上就是运行时堆栈。我们在这一阶段就是要实现这一映射功能，编写出 `loader()` 函数。我们先根据 ELF 头中的程序头表偏移量 `e_phoff` 定位程序头表。遍历程序头表中的每一项，如果可以加载，就把 ELF 中的程序段拷贝到该段的目标地址中，再把剩余空间清空。最后，`loader()` 会返回所装载程序的入口地址。
